...
from utils import machine_health
...
с круткой 50 кр/м. Нормы: разрывная нагрузка не менее 260 сН/текс; коэффициент вариации
не более 9% — партия с коэффициентом вариации выше 9% бракуется; линейная плотность 28,3–29,5 текс.

Напиши технологическую сводку на русском языке (250–400 слов) в Markdown.
Сводку читают люди, далёкие от статистики, — пиши понятно и без канцелярита.

АББРЕВИАТУРЫ И СОКРАЩЕНИЯ СТАТИСТИЧЕСКИХ ТЕРМИНОВ ЗАПРЕЩЕНЫ. Всегда пиши полностью:
- «коэффициент вариации» — никогда «CV»;
- «разрывная нагрузка» или «прочность» — никогда «РН»;
- «средний уровень за последние партии» — никогда «CL» или «центральная линия»;
- «индекс воспроизводимости процесса» — никогда «Cpk»; рядом поясни словами:
  больше 1,33 — запас есть, меньше 1,0 — процесс не держит допуск;
- «типичный размах колебаний» — никогда «сигма» или «σ»;
- сигналы контрольных карт описывай словами: вместо «8 точек ниже CL» пиши
  «показатель уже восемь партий подряд держится ниже обычного уровня — это не случайность,
  а системный сдвиг».
Заводские обозначения оборудования сохраняй: «ПМ 23» (прядильная машина),
«ВТВ №18» (аппарат термообработки термовытяжки).

ОБЯЗАТЕЛЬНАЯ СТРУКТУРА — все пять разделов присутствуют в каждой сводке:

1. **Общая оценка**: 🔴 критично / 🟡 есть локальные проблемы / 🟢 процесс в норме —
   одной фразой с главной причиной оценки.

2. **Полимер** — всегда явный вердикт: «проблема в полимере» либо «проблем с полимером нет».
   Обязательно аргументируй: сдвинулась ли вся партия целиком относительно среднего уровня
   и какая доля машин упала к своим обычным значениям. Логика: полимер один на всю партию,
   поэтому если упала вся партия широким фронтом — виноват полимер; если упали отдельные
   машины при нормальном среднем — полимер вне подозрений.

3. **Машины** — конкретный список проблемных машин: номер, цифры (отклонение от среднего
   партии, коэффициент вариации, диагноз из фактов) и конкретное действие — что проверить
   и кому передать (аппаратчик формования, оператор кручения и намотки, помощник мастера,
   сменный мастер — по справочнику). Если проблемных машин нет — прямо напиши «проблемных
   машин в этой партии нет».

4. **Аппараты ВТВ** — конкретные проблемные аппараты с цифрами и действиями
   (проверить температурный режим и время обработки по технологической карте, сообщить
   технологу участка). Если все аппараты в норме — прямо напиши это.

5. **Стабильность и динамика** — сигналы контрольных карт своими словами, индекс
   воспроизводимости процесса с пояснением, динамика брака за последние партии.

КАЖДЫЙ вывод аргументируй цифрами из фактов. Не «машина работает плохо», а
«ПМ 59 даёт разрывную нагрузку на 8 сН/текс ниже среднего партии, почти половина её
измерений ниже нормы 260 — поэтому диагноз "техсостояние" и нужна проверка узлов формования».
...
- адресуй рекомендации исполнителям, как указано в справочнике;
- НЕ называй нормы и узлы, которых нет в справочнике или фактах.

Если в фактах есть «диагнозы_машин» — расшифровка диагнозов:
«обслуживание» = низкая прочность и высокий коэффициент вариации одновременно — машина
запущена комплексно; «техсостояние» = устойчиво низкая прочность — износ узлов формования;
«кручение/намотка» = только высокий коэффициент вариации — натяжение, бегунок, веретено;
«деградация» = показатели пока в норме, но устойчивый тренд вниз — ранний сигнал.
...
def build_facts(df, thresholds, offset):
    ...
    # Диагнозы машин по накопительной ведомости (окно 20 партий)
    try:
        reg = machine_health.compute_registry(df, 20, thresholds)
        problems = reg[(reg['диагноз'] != 'норма') & (reg['активна'])].head(8)
        f['диагнозы_машин_за_20_партий'] = {
            f"ПМ {int(r['машина'])}": (
                f"{r['диагноз']} (откл. {r['откл_от_партии']:+.1f} сН/текс, CV {r['cv_средний']}%, "
                f"здоровье {r['здоровье']:.0f}/100)")
            for _, r in problems.iterrows()
        }
    except Exception:
        f['диагнозы_машин_за_20_партий'] = 'нет данных'
